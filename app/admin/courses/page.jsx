  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-background to-card/20 py-20 px-4 mt-42">
      ...
      <div className="container mx-auto max-w-7xl">
        ...
        <div ref={formRef} className="bg-card/50 backdrop-blur-sm rounded-3xl border border-primary/20 p-8 mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                ...
                rows={4}
                className="w-full px-4 py-3 rounded-xl bg-background border border-primary/20 focus:border-primary focus:outline-none text-foreground placeholder:text-foreground/40 transition-all resize-none"
              />
            </div>
            ...
            <div className="md:col-span-2">
              ...
              <div className="flex gap-3">
                <input
                  ...
                  className="flex-1 px-4 py-3 rounded-xl bg-background border border-primary/20 focus:border-primary focus:outline-none text-foreground placeholder:text-foreground/40 transition-all"
                />
                <button className="px-6 py-3 bg-secondary/10 text-secondary rounded-xl font-semibold hover:bg-secondary/20 transition-all duration-300 flex items-center gap-2 border border-secondary/20">
                  <Upload size={18} />
                  Upload
                </button>
              </div>
            ...
            <div>
              ...
              <select
                ...
                onChange={handleInputChange}
                className="w-full px-4 py-3 rounded-xl bg-background border border-primary/20 focus:border-primary focus:outline-none text-foreground transition-all appearance-none cursor-pointer"
              >
            ...
            <div>
              ...
              <input
                ...
                onChange={handleInputChange}
                className="w-full px-4 py-3 rounded-xl bg-background border border-primary/20 focus:border-primary focus:outline-none text-foreground transition-all"
              />
            </div>
          ...
          <div className="mt-8 flex gap-4">
            <button
              ...
              className="flex-1 px-8 py-4 bg-primary text-background rounded-full font-bold text-lg hover:bg-accent hover:shadow-lg hover:shadow-primary/30 transition-all duration-300 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 flex items-center justify-center gap-3"
            >
              {isSubmitting ? (
                <>
        ...
        <div className="bg-card/50 backdrop-blur-sm rounded-3xl border border-primary/20 p-8">
          ...
          {courses.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-xl text-foreground/60">No courses yet. Create your first course above!</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {courses.map((course) => (
                <div key={course._id} className="bg-background/50 rounded-2xl border border-primary/10 overflow-hidden hover:border-primary/30 transition-all duration-300 group">
                  ...
                  <div className="p-6">
                    <div className="flex items-center gap-2 mb-3">
                      <span className={`px-2 py-1 rounded-full text-xs font-semibold border ${getDifficultyColor(course.difficultyLevel)}`}>
                        {course.difficultyLevel}
                      </span>
                      <span className={`px-2 py-1 rounded-full text-xs font-semibold border ${getStatusColor(course.courseStatus)}`}>
                        {course.courseStatus}
                      </span>
                    </div>
                    ...
                    <p className="text-sm text-foreground/60 mb-1">{course.instructor}</p>
                    <p className="text-sm text-foreground/60 mb-4">Code: {course.code}</p>
                    
                    <div className="grid grid-cols-2 gap-2 text-sm text-foreground/70 mb-4">
                      <div>💰 ${course.price}</div>
                      <div>📚 {course.totalLessons} lessons</div>
                      <div>⏱️ {course.durationWeeks} weeks</div>
                      <div>👥 {course.enrolledStudents || 0}/{course.maxStudents || 'Unlimited'}</div>
                    </div>

                    <div className="flex gap-2">
                      <button
                        onClick={() => handleEdit(course)}
                        className="flex-1 px-4 py-2 bg-secondary/10 text-secondary rounded-lg font-semibold hover:bg-secondary/20 transition-all duration-300 flex items-center justify-center gap-2"
                      >
                        <Edit size={16} />
                        Edit
                      </button>
                      <button
                        ...
                        className="px-4 py-2 bg-accent/10 text-accent rounded-lg font-semibold hover:bg-accent/20 transition-all duration-300 flex items-center justify-center"
                      >
                        <Trash2 size={16} />
                      </button>
                    </div>
